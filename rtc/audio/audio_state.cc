#include "yealink/rtc/audio/audio_state.h"
...
webrtc::AudioProcessing* AudioState::audio_processing() {
  RTC_DCHECK(config_.audio_processing);
  return config_.audio_processing.get();
}

webrtc::AudioTransport* AudioState::audio_transport() {
...
void AudioState::SetPlayout(bool enabled) {
  ...
  if (playout_enabled_ != enabled) {
    ...
    if (enabled) {
      ...
      config_.audio_device_module->StopPlayout();
      null_audio_poller_ =
          absl::make_unique<webrtc::internal::NullAudioPoller>(&audio_transport_);
    }
  }
}
...
AudioState::Stats AudioState::GetAudioInputStats() const {
  RTC_DCHECK(thread_checker_.CalledOnValidThread());
  const webrtc::voe::AudioLevel& audio_level = audio_transport_.audio_level();
  Stats result;
