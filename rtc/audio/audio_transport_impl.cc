...
void ProcessCaptureFrame(uint32_t delay_ms,
  ...
  if (swap_stereo_channels) {
    webrtc::AudioFrameOperations::SwapStereoChannels(audio_frame);
  }
}

// Resample audio in |frame| to given sample rate preserving the
// channel count and place the result in |destination|.
int Resample(const webrtc::AudioFrame& frame,
...
AudioTransportImpl::AudioTransportImpl(
    ...
    : audio_processing_(audio_processing),
      mixer_(mixer),
      audio_session_(audio_session) {
  RTC_DCHECK(mixer);
  RTC_DCHECK(audio_processing);
  RTC_DCHECK(audio_session);
}
...
int32_t AudioTransportImpl::RecordedDataIsAvailable(
  ...
  RTC_DCHECK_GE(sample_rate,
                webrtc::AudioProcessing::NativeRate::kSampleRate8kHz);
  ...
  webrtc::voe::RemixAndResample(
      ...
      number_of_channels, sample_rate, &capture_resampler_, audio_frame.get());

  // TODO:
  // pass audio frame to audio session
  ...
  if (audio_processing_->GetConfig().voice_detection.enabled) {
    if (audio_frame->vad_activity_ != webrtc::AudioFrame::kVadUnknown) {
      bool vad_active =
  ...
  // encoding task will be posted internally to each stream.
  {
    ...
    if (!sending_streams_.empty()) {
      ...
      while (++it != sending_streams_.end()) {
        std::unique_ptr<webrtc::AudioFrame> audio_frame_copy(
            new webrtc::AudioFrame());
...
int32_t AudioTransportImpl::NeedMorePlayData(const size_t nSamples,
  ...
  // TODO
  // get audio frame from audio session
  ...
  *ntp_time_ms = mixed_frame_.ntp_time_ms_;
  ...
  RTC_DCHECK_EQ(error, webrtc::AudioProcessing::kNoError);
...
void AudioTransportImpl::PullRenderData(int bits_per_sample,
  ...
  if (use_audio_session_) {
    std::unique_ptr<webrtc::AudioFrame> audio_frame_dummy(
        new webrtc::AudioFrame());

    audio_frame_dummy->sample_rate_hz_ = sample_rate;
    audio_frame_dummy->num_channels_ = number_of_channels;
    audio_frame_dummy->samples_per_channel_ = number_of_frames;

    *elapsed_time_ms = audio_frame_dummy->elapsed_time_ms_;
    *ntp_time_ms = audio_frame_dummy->ntp_time_ms_;
...
void AudioTransportImpl::UpdateSendingStreams(
    std::vector<webrtc::AudioSendStream*> streams,
    int send_sample_rate_hz,
    size_t send_num_channels) {
  rtc::CritScope lock(&capture_lock_);
...
void AudioTransportImpl::SetUsingAudioSession(bool enable) {
  rtc::CritScope lock(&session_lock_);
  use_audio_session_ = enable;
}
}  // namespace yealink
